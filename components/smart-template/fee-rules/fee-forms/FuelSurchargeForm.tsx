...
export default function FuelSurchargeForm({ module, onSubmit, editingRule }: Props) {
  const labelPrefix = '燃油附加费';
  ...
  const currentRulePreview = isValid
    ? `当前规则：${labelPrefix}：${percentage}% of 运费`
    : '规则无效，请检查输入';
  ...
  const handleSubmit = () => {
    ...
    const rule: Omit<FeeRule, 'id'> & { id?: string } = {
      ...
      params: {
        ...
        baseField: '运费',
        label: `${labelPrefix}：${percentage}% of 运费`,
      },
      currency,
